...
namespace Drupal\stanford_events_importer\Form;
...
class EventsImporterForm extends ConfigFormBase {
  ...
  protected function getEditableConfigNames() {
    return ['stanford_events_importer.settings'];
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $urls = $this->config('stanford_events_importer.settings')->get('urls') ?: [];
    $form['urls'] = [
      '#type' => 'textarea',
      '#title' => $this->t('Urls'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->configFactory()
      ->getEditable('stanford_events_importer.settings')
    ...
    $role = $this->entityTypeManager->getStorage('user_role')
      ->load('site_manager');
    if ($role) {
      $role->grantPermission('import stanford_events_importer migration');
      $role->save();
    }
  }
