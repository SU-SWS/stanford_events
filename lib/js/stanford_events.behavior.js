...
export default {
  ...
  attach: function (context, settings) {
    (function ($) {
      ...
      const pastLabel =
        $('<span class="su-event-label-past">Past Event <span class="divider">|</span></span>');
      const pastText =
        $('<div class="su-event-text-past">This event has passed.</div>');

      // check the date on the page.
      const contentDate =
        parseInt(Date.parse($('div.su-event-date', eventDate).text()), 10);
