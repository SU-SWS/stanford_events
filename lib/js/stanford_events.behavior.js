...
export default {
  ...
  attach: function (context, settings) {
    (function ($) {
      ...
      const eventLabelLoc =
        $('div.section-editorial-content .centered-container');
      ...
      if (eventDate < todayBegin && eventTypeLabelLoc.length) {
        //Top label with a Type
        eventTypeLabel.prependTo(eventTypeLabelLoc.first());
        pastText.appendTo(eventTextLocation.last());
      }
      else {
        //Top label without a Type
        if (eventDate < todayBegin && (eventLabel.length)) {
          eventLabel.prependTo(eventLabelLoc); // Top label without a Type
          pastText.appendTo(eventTextLocation.last());
        }
      }
